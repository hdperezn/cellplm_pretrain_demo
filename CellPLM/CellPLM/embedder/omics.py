...
class OmicsEmbeddingLayer(nn.Module):
    ...
    #         pe = self.pe_enc(pe_input)#0.#self.pe_enc(pe_input)
    ...
    #     return x
    def forward(self, x_dict, input_gene_list=None):
        # 1) base expression embedding -----------------------------------------
        x = self.feat_enc(x_dict, input_gene_list)        # [N × H]

        # 2) positional encoding (if requested) --------------------------------
        if self.pe_enc is not None:
            pe_input = x_dict[self.pe_enc.pe_key].to(x.device)  # ensure same device
            pe = self.pe_enc(pe_input)                          # should return Tensor

            # if the encoder still gave float, convert it
            if not torch.is_tensor(pe):
                pe = torch.as_tensor(pe, dtype=x.dtype, device=x.device)

            # optional covariate injection
            if self.inject_covariate:
                pe = pe + self.cov_enc(x_dict['batch'])

            # concatenate or add
            x = torch.cat([x, pe], 1) if self.cat_pe else x + pe ## i modify this
